...
class Person:
    ...
    def show_worst_frame(self, by='others', show=True):
        """
...
class PersonsContainer:
    ...
    def show_exposed(self, by='others'):
        """
        ...
        for i, person in enumerate(top):
            plt.subplot(2, 2, i+1)
            person.show_worst_frame(by=by, show=False)
        plt.tight_layout()
        plt.show()
        return top
    ...
    def hist(self, by='others', ignore_safe=False):
        ...
        Input
            ...
            ignore_safe: boolean indicating whether to include or not
                         ...
                         Defaults to False.
        """
        data = self.histdata(by, ignore_safe)
        avg_ = data.mean()
        max_ = data.max()
        if by == 'time':
...
class FramesContainer:
    ...
    def check_if_worst(self, frame, picture):
        """
    ...
    def worst_frame(self):
        ...
        plt.title(f'Frame #{frame.number}' +
                  ...
                  f'{frame.num_at_risk} persons in danger')
        plt.show()
        return frame
